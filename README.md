...
下書きが尽きた日には、［出す］→［予備］に作り置きしてあるものから2件を提案します。
...
| タブ | 中身 | いつ開くか |
|---|---|---|
| **出す** | 今日 / 今週 / 予備 | 毎日。ふだん開くのはここだけ |
| **つくる** | 宣伝ポストの注文、返信の下書きの依頼、作った投稿 | 出したいものが予定に無いとき |
| **note** | リポジトリに用意された記事、週の下書き | 週に1回 |
| **記録** | 投稿ずみ / 過去、反応をまとめて送る | 出したあと |

タブには数字が出ます（今日の残り・待っている注文・まだ送っていない記録）。
**開かないと分からないことを、開く前に出す**ためです。

カードにいつも出ているボタンは **［𝕏 に共有］［次へ］［投稿した］** と **［…］** だけです。
たまにしか使わないもの（コピーして X を開く・本文を直す・別の案を見る・画像を保存）は
［…］の中にまとめてあります。毎日押す2つと、たまに押す4つが同じ大きさで並んでいると、
毎日押すほうを毎回さがすことになるためです。

> 通知の Issue に残っている `#now` や `#done` のリンクは、これまでどおり効きます。
> タブは4つになりましたが、中身の分けかた（view）は7つのままにしてあるためです。
